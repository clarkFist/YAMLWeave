            ...
            class MinimalStubProcessor:
                ...
                def process_single_file(self, content, file_path, callback=None):
                    ...
                    try:
                        ...
                        if self.ui:
                            self.ui.root.after(0, self.ui.log, f"[处理] 正在处理: {os.path.basename(file_path)}")
                        ...
                        while i < total_lines:
                            ...
                            if "//" in line and "TC" in line and "STEP" in line:
                                ...
                                if len(parts) >= 2 and parts[0].startswith("TC") and parts[1].startswith("STEP"):
                                    tc_id = parts[0]
                                    step_id = parts[1]
                                    
                                    # 获取segment ID (可能是segment1或特定名称)
                                    segment_id = parts[2] if len(parts) > 2 else None
                                    if segment_id and ":" in segment_id:
                                        segment_id = None  # 如果包含冒号，不是segment ID
                ...
                def process_files(self):
                    ...
                    try:
                        ...
                        if not project_dir or not os.path.isdir(project_dir):
                            error_msg = f"项目目录无效: {project_dir}"
                            logger.error(error_msg)
                            if self.ui:
                                self.ui.root.after(0, self.ui.log, f"[错误] {error_msg}")
                        ...
                        self.stats["scanned_files"] = total_files
                        if total_files == 0:
                            warning_msg = f"目录中未找到C文件: {project_dir}"
                            logger.warning(warning_msg)
                            if self.ui:
                                self.ui.root.after(0, self.ui.log, f"[警告] {warning_msg}")
                                self.ui.root.after(0, self.ui.update_status, "未找到C文件")
                            return False, warning_msg, self.stats
                        # 处理每个文件
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    with open(file_path, 'r', encoding='gbk') as f:
                                        content = f.read()
                                updated_content = self.process_single_file(content, file_path)
                                # 如有更新，写回文件
                                ...
                                if self.ui:
                                    import datetime
                                    print(f"[DEBUG][{datetime.datetime.now()}] before after: percent={percent}, file={file_path}")
                                    self.ui.root.after(0, self.ui.update_progress, percent, os.path.basename(file_path), i + 1, total_files)
                                    print(f"[DEBUG][{datetime.datetime.now()}] after after: percent={percent}, file={file_path}")
                            except Exception as file_err:
                                logger.error(f"处理文件失败: {file_path}, 错误: {str(file_err)}")
                                self.stats["failed_files"] += 1
                                if self.ui:
                                    self.ui.root.after(0, self.ui.log, f"[错误] 处理文件失败: {os.path.basename(file_path)}")
                        # 处理完成，进度100%
                        if self.ui:
                            import datetime
                            print(f"[DEBUG][{datetime.datetime.now()}] before after: percent=100, file=处理完成")
                            self.ui.root.after(0, self.ui.update_progress, 100, "处理完成", total_files, total_files)
                            print(f"[DEBUG][{datetime.datetime.now()}] after after: percent=100, file=处理完成")
                            self.ui.root.after(0, self.ui.update_status, "处理完成")
                            self.ui.root.after(0, self.ui.log, "[成功] 处理完成")
                        return True, "处理完成", self.stats
                    except Exception as e:
                        logger.error(f"处理文件时出错: {str(e)}")
                        if self.ui:
                            self.ui.root.after(0, self.ui.log, f"[错误] 处理文件时出错: {str(e)}")
                            self.ui.root.after(0, self.ui.update_status, "处理失败")
        ...
        try:
            ...
            if app_ui is not None and os.path.exists(example_project_path):
                ...
                try:
                    ...
                    # 添加备份功能介绍
                    app_ui.log("[信息] ========================================")
                    app_ui.log("[信息] 项目备份功能已启用:")
                    app_ui.log("[信息] 1. 在处理项目前自动创建整个项目的备份")
                    app_ui.log("[信息] 2. 插桩后的文件保存在新目录中")
                    app_ui.log("[信息] 3. 原始项目保持不变")
                    app_ui.log("[信息] 4. 备份目录名: 项目目录名_backup_时间戳")
                    app_ui.log("[信息] 5. 插桩结果目录名: 项目目录名_stubbed_时间戳")
                    app_ui.log("[信息] ========================================")
                    ...
                    try:
                        ...
                        if total_logs > 0:
                            app_ui.log("[信息] ========================================")
                            app_ui.log("[信息] 执行日志历史统计:")
                            app_ui.log(f"[信息] 总执行次数: {total_logs} 次")
                            ...
                            if total_processed_files > 0:
                                app_ui.log(f"[信息] 总处理文件数: {total_processed_files} 个")
                            
                            if total_inserted_stubs > 0:
                                app_ui.log(f"[信息] 总插入桩点数: {total_inserted_stubs} 个")
                            
                            latest_log = log_stats.get("latest_log")
                            if latest_log:
                                app_ui.log(f"[信息] 最近执行时间: {latest_log.get('modified_time_str', '未知')}")
                            
                            # 执行日志目录位置
                            exec_logs_dir = os.path.join(APP_ROOT, "execution_logs")
                            app_ui.log(f"[信息] 执行日志保存在: {exec_logs_dir}")
                            app_ui.log("[信息] ========================================")
