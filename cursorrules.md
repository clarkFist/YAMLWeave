...
1. **插桩结果保存机制**：
   - 系统自动在源码目录的同级目录下创建时间戳标记的"源目录名_backup"格式备份目录
   - 对于示例目录，源目录指的就是samples目录
   - 同时生成时间戳标记的"源目录名_stub"格式插桩结果目录
   - 完整保留原始文件结构，仅针对包含锚点的文件执行处理操作
   - 确保原始源代码文件不受任何影响，有效避免对原始代码的意外修改风险
   注意：举出一个例子：
   输入路径c:\Users\clark\Desktop\YAMLWeave\samples，它会创建：
c:\Users\clark\Desktop\YAMLWeave\samples_backup_20240615_123456
c:\Users\clark\Desktop\YAMLWeave\samples_stubbed_20240615_123456
...
4. **错误处理**：
   - 优雅处理各种异常情况
   - 提供用户友好的错误提示
我需要基于现有代码整理一份需求文档，描述代码的输入、输出以及各种格式。
